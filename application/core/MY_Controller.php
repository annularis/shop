...
class MY_Controller extends CI_Controller
{
    ...
    protected function _render($template, $data = NULL)
    {
        ...
        if ($data != NULL) {
            ...
            foreach ($this->_template_data_array as $key => $value) {
                ...
                $this->smartie->assign($key, $value);
            }
        }

        $this->smartie->assign('header', array('title' => $page_title,
        ...
        $this->smartie->assign('footer', array('price_index' => $this->bw_config->price_index,
       ...
       $this->smartie->display($template . ".tpl");
    ...
    protected function _prepare_template()
    {
       ...
       $this->smartie->assign('current_user', $this->current_user->status(), 'global');
        $this->smartie->assign('allow_guests', $this->bw_config->allow_guests);
        $this->smartie->assign('coin', $this->bw_config->currencies[0]);
        ...
        if ($this->current_user->logged_in()) {
            $this->smartie->assign('count_unread_messages', $this->general_model->count_unread_messages(), 'global');
            if ($this->current_user->user_role == 'Vendor')
                $this->smartie->assign('count_new_orders', $this->general_model->count_new_orders(), 'global');
        ...
        $this->smartie->assign('category_data', $category_data); 
    }
    ...
    public function _handle_partials()
    {
        ...
        if (count($this->_partials) > 0) {
            foreach ($this->_partials as $variable_name => $page_to_render) {
                $a = $this->smartie->fetch($page_to_render . ".tpl");
                $this->smartie->assign($variable_name, $a);
            }
        }
    }
}
