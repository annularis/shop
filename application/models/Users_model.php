...
class Users_model extends CI_Model
{
    ...
    public function get(array $user)
    {
        ...
        $key = $key[0];
        if (in_array($key, array('user_hash', 'id', 'user_name'))) {
            $query = $this->db->select('id, banned, user_hash, user_name, local_currency, user_role, salt, force_pgp_messages, public_key, pgp_two_factor, totp_two_factor, entry_paid, email_address,  activation_hash, activation_id')
